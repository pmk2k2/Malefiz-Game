...
public class GameIntegrationTest {
        ...
        void testGameWorkflow() throws Exception {
                ...
                String gameCode = objectMapper.readValue(createResult.getResponse().getContentAsString(), Map.class)
                                .get("gameCode").toString();
                ...
                // 3. Spieler auf Ready setzen
                // a. Host (erster Spieler)
                Map<String, String> hostReadyBody = Map.of("code", gameCode, "playerId", hostPlayerId, "isReady",
                                "true");
                mockMvc.perform(post("/api/game/setReady")
                                .session(hostSession)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(hostReadyBody)))
                                .andExpect(status().isOk());

                // b. zweiter Spieler
                Map<String, String> spielerReadyBody = Map.of("code", gameCode, "playerId", "2", "isReady", "true");
                mockMvc.perform(post("/api/game/setReady")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(spielerReadyBody)))
                                .andExpect(status().isOk());

                // 4. Spiel starten (durch Host)
                // Body mit PlayerID, um die Host-Autorisierung zu erfüllen + gameCode
                Map<String, String> startBody = Map.of("code", gameCode, "playerId", hostPlayerId);
                ...
                mockMvc.perform(get("/api/game/" + gameCode + "/figures"))
                                ...
                                .andExpect(jsonPath("$[5].color", is("#ffff00"))); // Spieler 2 Farbe prüfen (Index 5
                                                                                   // ist die 1. Figur des 2. Spielers)
        }
}
